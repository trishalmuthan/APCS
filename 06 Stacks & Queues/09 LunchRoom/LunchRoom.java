...
public class LunchRoom
{
   ...
   public static void main(String[] args)
   {
      ...
      /*We initialize a bunch of double and int variables to store various calculations that we will have to make*/
      double SRtotalwaittime = 0;
      double JRtotalwaittime = 0;
      double SOtotalwaittime = 0;
      double FRtotalwaittime = 0;
      ...
      /*We create a serviceTime variable that will store a randomly generated service time from 2 to 7 minutes and will be used to give each Customer object a certain service time
      as their field. First, we have a for loop that runs until ones less than TIME (1080 min). In this for loop we have an if statement that checks if the queue is empty. If it 
      is empty, then we generate, using Math.random(), a service time from 2 to 7 and store it in the serviceTime variable. Next, we check if entered() returns true. entered() is
      basically a method that returns true if someone has entered in our simulation. If someone does enter, we add a new Customer object to the priority queue which has the
      argument x, the current minute, getWhichYear(), a method that randomly gives us a grade level, and serviceTime.*/
      int serviceTime = 0;
      for(int x = 0; x < TIME; x++)
      {
         ...
         if(entered())
         {
            queue.add(new Customer(x, getWhichYear(), serviceTime));
         }
          
        /*Next, we call display with x, the current minute, and the queue. display() will print the full line of customers for each minute. Then, we have an if statement
        that checks if the queue is empty. If it isn't then we check if there is still time remaining on the customer at the front of the line's service time and if there
        is, then we decrease this service time. If this isn't the case, then we go to the else if, which would mean the service time remaining is equal to 0. Within this,
        we remove the first customer and store it in a variable. Then we check what year it is. Within each year if statement, we calculate the currentWaitTime by subtracting
        the current minute from the minute that they entered at. This is added to the statistic for total wait time for the specific grade level variable. Then, we increase
        the grade level variable for number of customers by one. Lastly, we check if the current wait time is longer than the specific grade level variable that holds the longest
        wait time so far. If it is longer, you set that variable to currentWaitTime. Then we just set serviceTime to something new from 2-7.*/
         display(x, queue);
         if(!queue.isEmpty())
         {
            if(queue.peek().getServe() != 0)
            {
               queue.peek().decreaseServe();
            }
            else if(queue.peek().getServe() == 0)
            {
               Customer store = queue.remove();
               if(store.getYear() == 20)
               { 
                  int currentWaitTime = x - store.getEntry();
                  SRtotalwaittime += currentWaitTime;
                  SRcustomers++;
                  if(currentWaitTime > SRlongest)
                  {
                     SRlongest = currentWaitTime;
                  }
                  serviceTime = (int) (Math.random() * 6) + 2;
               }
               
               else if(store.getYear() == 21)
               { 
                  int currentWaitTime = x - store.getEntry();
                  JRtotalwaittime += currentWaitTime;
                  JRcustomers++;
                  if(currentWaitTime > JRlongest)
                  {
                     JRlongest = currentWaitTime;
                  }
                  serviceTime = (int) (Math.random() * 6) + 2;
               }
               
               else if(store.getYear() == 22)
               { 
                  int currentWaitTime = x - store.getEntry();
                  SOtotalwaittime += currentWaitTime;
                  SOcustomers++;
                  if(currentWaitTime > SOlongest)
                  {
                     SOlongest = currentWaitTime;
                  }
                  serviceTime = (int) (Math.random() * 6) + 2;
               }
               ...
               { 
                  int currentWaitTime = x - store.getEntry();
                  FRtotalwaittime += currentWaitTime;
                  FRcustomers++;
                  if(currentWaitTime > FRlongest)
                  {
                     FRlongest = currentWaitTime;
                  }
                  serviceTime = (int) (Math.random() * 6) + 2;
               }
            }
      ...
      /*We calculate the averages and print out all the required statistics for the lab.*/
      SRaverage = SRtotalwaittime/SRcustomers;
      JRaverage = JRtotalwaittime/JRcustomers;
      SOaverage = SOtotalwaittime/SOcustomers;
      FRaverage = FRtotalwaittime/FRcustomers;
      
      System.out.println("Customer\t\tTotal\t\tLongest\t\tAverage Wait");
      System.out.println("Senior\t\t" + SRcustomers + "\t\t" + SRlongest + "\t\t" + SRaverage);
      System.out.println("Junior\t\t" + JRcustomers + "\t\t" + JRlongest + "\t\t" + JRaverage);
      System.out.println("Sophomore\t\t" + SOcustomers + "\t\t" + SOlongest + "\t\t" + SOaverage);
      System.out.println("Freshman\t\t" + FRcustomers + "\t\t" + FRlongest + "\t\t" + FRaverage);
      /* report the data  */
   
   }
   
   /*We create a string that stores the minute and the bracket. Then, we check if q is not empty and if it isn't, we use a for each loop to add each Customer's toString and a 
   comma. Then, we get rid of the comma and space at the end using substring. Lastly, we add an ending bracket and return.*/
   ...
   /*We just return true if Math.random is less than 0.20 and return false otherwise.*/
   ...
   /*We have doubles that store the percentage of each. Then we calculate math.random and check which range it is in. Depending on the range that math.random is located in, a 
   different grade level is returned. All of these grades have an equal chance of being selected.*/
   public static int getWhichYear()
   {
      double twenty = 0.25;
      double twentyone = 0.25;
      double twentytwo = 0.25;
      double twentythree = 0.25;
      ...
      if(random < twenty + twentyone)
      {
         return 21;
      }
        
      if(random < twenty + twentyone + twentytwo)
      {
         return 22;
      }
...
/*The Customer class has 3 private variables, the minute they entered at, the grade they are, and the service time. This constructor takes 3 arguments and sets these 3 variables.
We have get methods for all 3 variables, a decreaseServe method which subtracts 1 minute from serveTime, and a toString, which compiles the minute and the correct letter
abbreviation for the object. Then, we have a compareTo method. This method first checks for year, and if the years end up being the same, then we also look at the minute at which
they entered so we can maintain first in first out.*/
class Customer implements Comparable<Customer>
{
   private int timeOfEntry;
   private int year;
   private int serveTime;
   public Customer(int time, int yearEnter, int serve)
   {
      timeOfEntry = time;
      year = yearEnter;
      serveTime = serve;
